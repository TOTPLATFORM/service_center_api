...
public class OfferServiceTest
{
    ...
    [InlineData(20, "2000-12-30", "2000-12-30", "BuyOneGetOne", "offer is done", 1)]
    ...
    public async Task GetAllOffer()
    {
        ...
        // Act
        var result = await _offerService.GetAllOfferAsync(2,1);
    ...
    [InlineData(1)]
    [InlineData(10)]
    public async Task GetByIdOffer(int id)
    {
        // Arrange
        CheckService();
    ...
    [InlineData(1, 20, "2000-12-30", "2000-12-30", "BuyOneGetOne", "offer is done", 1, true)]
    [InlineData(20, 20, "2000-12-30", "2000-12-30", "BuyOneGetOne", "offer is done", 1, false)]
    ...
    /// <summary>
    /// Tests the search functionality in the offer service to ensure it can find offer based on a search term.
    /// </summary>
    [Fact, TestPriority(5)]
    public async Task GetOffersForSpecificProduct()
    {
        // Arrange
        CheckService();
        int productId = 1;
        ...
        // Act
        var result = await _offerService.GetProductsByOffer(productId, 2, 1);

        // Assert
        Assert.True(result.IsSuccess);
    }
    /// <summary>
    /// Tests the search functionality in the offer service to ensure it can find offers based on a search term.
    /// </summary>
    [Fact, TestPriority(4)]
    public async Task SearchOffer()
    {
        // Arrange
        CheckService();
        string text = "offer is Done";

        // Act
        var result = await _offerService.SearchOfferByTextAsync(text, 2, 1);
