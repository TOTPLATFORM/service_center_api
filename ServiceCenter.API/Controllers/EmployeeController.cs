...
public class EmployeeController(IEmployeeService employeeService) : BaseController
{
	...
	[HttpGet]
	[Authorize(Roles = "Admin,Manager")]
	[ProducesResponseType(typeof(Result<PaginationResult<EmployeeResponseDto>>), StatusCodes.Status200OK)]
	public async Task<Result<PaginationResult<EmployeeResponseDto>>> GetAllEmployees(int itemCount, int index)
	{
		return await _employeeService.GetAllEmployeesAsync(itemCount,index);
	}
	/// <summary>
	/// get all employees in the system.
	/// </summary>
	...
	public async Task<Result<EmployeeGetByIdResponseDto>> GetEmployeeById(string id)
	{
	...
	[ProducesResponseType(typeof(Result<EmployeeResponseDto>), StatusCodes.Status200OK)]
	[ProducesResponseType(typeof(Result), StatusCodes.Status404NotFound)]
	public async Task<Result<PaginationResult<EmployeeResponseDto>>> SerachEmployeeByText(string text, int itemCount, int index)
	{
		return await _employeeService.SearchEmployeeByTextAsync(text,itemCount,index);
	}
	/// <summary>
